...
NGQgisApp::NGQgisApp(
  QSplashScreen *splash, AppOptions options, const QString &rootProfileLocation, const QString &activeProfile, QWidget *parent, Qt::WindowFlags fl
)
...
void NGQgisApp::checkQgisVersion()
{
  ...
  QgsMessageLog::logMessage( tr( "Started check updates ..." ), QString(), Qgis::Info );
#ifdef HAVE_NGSTD
...
void NGQgisApp::createToolBars()
{
  ...
  QString endPointStr = settings.value( "nextgis/endpoint", NGAccess::instance().endPoint() ).toString();
  QString authEndPointStr = settings.value( "nextgis/auth_endpoint", NGAccess::instance().authEndpoint() ).toString();
  QString tokenEndPointStr = settings.value( "nextgis/token_endpoint", NGAccess::instance().tokenEndpoint() ).toString();
...
void NGQgisApp::functionProfileNG( void ( NGQgisApp::*fnc )(), NGQgisApp *instance, QString name )
{
  startProfile( name );
  ( instance->*fnc )();
  endProfile();
