...
static QString platformStr()
{
  QString os;
#ifdef Q_OS_LINUX
  os = QString( "Linux (%1)" ).arg( ( QSysInfo::WordSize == 32 ) ? "i386" : ( QSysInfo::WordSize == 64 ) ? "amd64"
                                                                                                         : "unknown" );
...
NgsAboutDialog::NgsAboutDialog( QWidget *parent )
  : QDialog( parent ), ui( new Ui::NgsAboutDialog )
{
  ui->setupUi( this );

  setWindowFlag( Qt::MSWindowsFixedSizeDialogHint, true );

  QString version( NEXTGIS " QGIS" );
  ...
  details += NEXTGIS " " + tr( "QGIS version" ) + ": " + Qgis::ngqFullVersion() + "<br>";
  ...
  details += QStringLiteral( POSTGRESQL_VERSION );
#else
  ...
  details += "<a href='" + QgsNgUtils::nextgisDomain() + "'><b>NextGIS</b></a>";
